...
class RadiusFinder(object):
    ...
    def find_circles(self, img_path, binary_threshold=(25, 100), gaussian_kernel_size=5, gaussian_blur=0, dp=2.4, minDist=40,
                     minRadius=10, maxRadius=80):
        self.img_path = img_path
        self.img = cv2.imread(img_path)
        self.orginial = self.img.copy()
        ...
        self.imshow()
        ans = dict(diameters=diameters.tolist(), total=len(diameters), img_data=self.img_to_base64(self.img))
        return json.dumps(ans)

    def _preprocess_image(self, binary_threshold, gaussian_kernel_size, gaussian_blur):
    ...
    def imshow(self):
        if self.circles is not None:
            ...
            # cv2.imshow("output", np.hstack([self.orginial, self.img]))
            # cv2.waitKey(0)
...
if __name__ == '__main__':
    rf = RadiusFinder()
    asdf = rf.find_circles('/home/hjortur/Downloads/400gxCond1_Image001_ch00.tif')
    rf.asdf('/home/hjortur/Downloads/400gxCond1_Image001_ch00.tif')
